"""Remotely control your CoinW account via their API https://www.coinw.com/front/API """
...
class AuthAPI(AuthAPIBase):
    def __init__(
        self, api_key="", api_secret="", api_passphrase="", api_url="https://api.coinw.com", cache_path="cache", use_cache=False, app: object = None
    ) -> None:
        """coinw API object model

        Parameters
        ----------
        api_key : str
            Your coinw account portfolio API key
        api_secret : str
            Your coinw account portfolio API secret
        api_passphrase : str
            Your coinw account portfolio API passphrase
        api_url
            coinw API URL
        """
        ...
        if not p.match(api_key):
            self.handle_init_error("coinw API key is invalid", app=app)

        # validates the api secret is syntactically correct
        p = re.compile(r"^[A-z0-9-]{36,36}$")
        if not p.match(api_secret):
            self.handle_init_error("coinw API secret is invalid", app=app)

        # validates the api passphase is syntactically correct
        p = re.compile(r"^[A-z0-9#$%=@!{},`~&*()<>?.:;_|^/+\[\]]{8,32}$")
        if not p.match(api_passphrase):
            self.handle_init_error("coinw API passphrase is invalid", app=app)
        ...
        if use_cache:
            ...
            self._cache_filepath = cache_path + os.path.sep + "coinw_order_cache.json"
            self._cache_lock_filepath = cache_path + os.path.sep + "coinw_order_cache.lock"

        self.usecoinwcache = use_cache
    ...
    def get_account(self, account: str) -> pd.DataFrame:
        ...
        if not p.match(self.account):
            self.handle_init_error("coinw account is invalid", app=self.app)
    ...
    def get_orders(self, market: str = "", action: str = "", status: str = "all") -> pd.DataFrame:
        ...
        if market != "":
            ...
            if not self._is_market_valid(market):
                raise ValueError("coinw market is invalid.")
        ...
        # Update Cache if needed before continuing on any bot for coinw
        if self.usecoinwcache:
            self.buildOrderHistoryCache()

        # GET /orders?status
        resp = self.auth_api("GET", f"api/v1/orders?symbol={market}", use_order_cache=self.usecoinwcache, use_pagination=self.usepagination)
        if len(resp) > 0:
            if status == "active":
                df = resp.copy()[
                    [
        ...
        while trycnt <= connretry:
            try:
                ...
                if resp.status_code == 200 and len(resp.json()) > 0:
                    ...
                    msg = f"coinw auth_api Error: {method.upper()} ({resp.status_code}) {self._api_url} {uri} - {resp.json()['msg']}"
            ...
            if trycnt >= maxretry:
                if reason in ("ConnectionError", "HTTPError") and trycnt <= connretry:
                    ...
                    if msg is None:
                        msg = f"Unknown coinw Private API Error: call to {uri} attempted {trycnt} times, resulted in error"
                    if reason is None:
            ...
            return self.handle_api_error(f"coinw API Error: call to {uri} attempted {trycnt} times without valid response", "coinw Private API Error")
...
class PublicAPI(AuthAPIBase):
    def __init__(self, api_url: str = "https://api.coinw.com", app: object = None) -> None:
        # options
        self.die_on_api_error = False
        self._api_url = api_url
        ...
        valid_urls = [
            "https://api.coinw.com",
            "https://api.coinw.com/",
            "https://openapi-sandbox.coinw.com",
            "https://openapi-sandbox.coinw.com/",
        ]
    ...
    def get_historical_data(
        ...
        # validates the market is syntactically correct
        if not self._is_market_valid(market):
            raise TypeError("coinw market required.")
        ...
        if websocket is None or (websocket is not None and using_websocket is False):
            ...
            while trycnt < maxretry:
                ...
                if "code" in resp and resp["code"] != "200000":
                    raise ValueError(f"coinw API error: {resp['msg']} ({market})")
                ...
                try:
                    if "data" in resp:
                        ...
                        if trycnt >= (maxretry):
                            raise Exception(f"coinw API Error for Historical Data - attempted {trycnt} times - API did not return correct response")
                        time.sleep(15)

                except Exception as err:
                    if trycnt >= (maxretry):
                        raise Exception(f"coinw API Error for Historical Data - attempted {trycnt} times - Error: {err}")
                    time.sleep(15)
    ...
    def get_ticker(self, market: str = DEFAULT_MARKET, websocket=None) -> tuple:
        ...
        while trycnt <= maxretry:
            try:
                ...
                if trycnt >= maxretry:
                    if self.app:
                        RichText.notify(f"coinw API Error for Get Ticker - attempted {trycnt} times - Error: {err}", self.app, "warning")
        ...
        while trycnt <= connretry:
            ...
            if trycnt >= maxretry:
                if reason in ("ConnectionError", "HTTPError") and trycnt <= connretry:
                    ...
                    if msg is None:
                        msg = f"Unknown coinw Public API Error: call to {uri} attempted {trycnt} times, resulted in error"
                    if reason is None:
                        reason = "Unknown Error"
                    return self.handle_api_error(msg, reason)
            else:
            ...
            return self.handle_api_error(f"coinw API Error: call to {uri} attempted {trycnt} times without valid response", "coinw Public API Error")
...
class WebSocket(AuthAPIBase):
    def __init__(
        ...
        # granularity=None,
        granularity: Granularity = Granularity.ONE_HOUR,
        api_url="https://api.coinw.com",
        ws_url="wss://ws-api.coinw.com",
        app: object = None,
    ) -> None:
        valid_urls = [
            "https://api.coinw.com",
        ...
        valid_ws_urls = [
            "wss://ws-api.coinw.com",
        ]

        # validate Coinbase Pro Websocket URL
        if ws_url not in valid_ws_urls:
            raise ValueError("coinw WebSocket URL is invalid")
...
class WebSocketClient(WebSocket):
    def __init__(
        ...
        granularity: Granularity = Granularity.ONE_HOUR,
        api_url="https://api.coinw.com/",
        ws_url: str = "wss://ws-api.coinw.com",
        app: object = None,
    ) -> None:
        if len(markets) == 0:
